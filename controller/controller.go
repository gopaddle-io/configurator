...
func NewController(
	...
	// owned by a configMap resource will enqueue that configMap resource for
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		if ownerRef.Kind != "ConfigMap" {
			...
			c.enqueueConfigurator(configmap)
		}
		return
	}
}
...
func (c *Controller) processNextWorkItem() bool {
	...
	err := func(obj interface{}) error {
		...
		if event.Kind == "ConfigMap" {
			klog.Infof("kind: ConfigMap Name of the resource ", event.NameAndNameSpace)
			if err := c.configMapSyncHandler(event.NameAndNameSpace); err != nil {
